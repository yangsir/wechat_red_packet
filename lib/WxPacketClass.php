...
include_once("SdkExtraClass.php");

class WxPacketClass {

	protected $parameters; //cft 参数
    protected $app_sign; //商户秘钥

	function __construct($app_sign){
	...
	protected function get_sign(){
		try {
			if($this->check_sign_parameters() == false) {   //检查生成签名参数
			   throw new SDKException("生成签名参数缺失！" . "<br>");
		    }
			...
			return $md5SignUtil->sign($unSignParaString,$commonUtil->trimString($this->app_sign));
	...
	*/
	...
	function curl_post_ssl($url, $vars, $second=30,$aHeader=array()) {
		...
		$data = curl_exec($ch);
        file_put_contents('a.log',var_export($data,true));
		if($data){
			curl_close($ch);
			return $data;
		} else { 
